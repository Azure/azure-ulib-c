...
TEST_THREAD_RESULT test_thread_create(
    THREAD_HANDLE* thread_handle,
    THREAD_START_FUNC func,
    void* arg)
{
  TEST_THREAD_RESULT result;
  if ((thread_handle == NULL) || (func == NULL))
  {
...
void test_thread_exit(int res) { ExitThread(res); }

void test_thread_sleep(unsigned int milliseconds) { Sleep(milliseconds); }
