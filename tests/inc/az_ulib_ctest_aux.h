...
static int uint8_t_ptr_compare(const uint8_t_ptr left, const uint8_t_ptr right, size_t size)
{
  int result;
...
static void uint8_t_ptr_to_string(char* string, size_t buffer_size, uint8_t_ptr buf, size_t size)
{
...
#define ASSERT_BUFFER_ARE_EQUAL(type, A, B, size, ...)                                         \
  ...
  {                                                                                            \
    ...
    MU_C2(type, _to_string)                                                                    \
    (expected_string, sizeof(expected_string), A_value, size); /*one evaluation per argument*/ \
    MU_C2(type, _to_string)                                                                    \
    (actual_string, sizeof(actual_string), B_value, size); /*one evaluation per argument*/     \
    ...
    {                                                                                          \
      ...
      (void)snprintf(                                                                          \
          temp_str,                                                                            \
          sizeof(temp_str),                                                                    \
          "  Assert failed: Expected: %s, Actual: %s\n",                                       \
          expected_string,                                                                     \
          actual_string);                                                                      \
    ...
    uint8_t expected_content_length)
{
  ...
  if (offset < expected_content_length)
  {
    ...
    ASSERT_BUFFER_ARE_EQUAL(
        uint8_t_ptr, (const uint8_t* const)(expected_content + offset), buf_result, size_result);
  }

  size_result = 10;
  ASSERT_ARE_EQUAL(
      int, AZ_ULIB_EOF, az_ulib_ustream_read(ustream_instance, buf_result, 256, &size_result));
