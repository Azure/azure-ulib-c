 ...
 */
static inline az_result math_create(math_handle handle)
{
 ...
 */
static inline az_result math_sum(math_handle handle, int32_t a, int32_t b, int64_t* res)
{
  // Marshalling
  sum_model_in in = { a, b };

  // Call
  return az_ulib_ipc_call(handle, MATH_INTERFACE_SUM_COMMAND, &in, res);
}

/*
 * Azure Callable Wrapper for math subtract.
 */
static inline az_result math_subtract(math_handle handle, int32_t a, int32_t b, int64_t* res)
{
  // Marshalling
  subtract_model_in in = { a, b };

  // Call
  return az_ulib_ipc_call(handle, MATH_INTERFACE_SUBTRACT_COMMAND, &in, res);
}
