 ...
 *      3) Concatenate the second ustream to the first ustream. Dispose of the local instance of
 *              ustream_two. Remember we must do this to avoid memory leaks because while
 *              ustream_one is copied into the az_ulib_ustream_multi_data_cb (still one reference),
 *              ustream_two is cloned (meaning there are now two references). We then print the
 *              size of the concatenated ustream.
 *      4) Print the concatenated ustream. Dispose of the ustream_one instance. Underneath the
 *              covers, since references to both buffers reaches zero, the free functions for the
 *              buffers and the control blocks are called and no memory is leaked.
 */
...
int main(void)
{
  ...
  else
  {
    ...
    else
    {
      ...
      // Create the second az_ulib_ustream from the string in the heap, passing standard free
      // function as release callback
      az_ulib_ustream ustream_two;
      az_ulib_ustream_data_cb* ustream_control_block_two
          = (az_ulib_ustream_data_cb*)malloc(sizeof(az_ulib_ustream_data_cb));
      if ((result = az_ulib_ustream_init(
               &ustream_two,
               ustream_control_block_two,
               free,
