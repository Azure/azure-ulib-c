...
#define CONTOSO_PACKAGE_VERSION 200401
...
void contoso_display_200401_create(void)
{
  az_result result;
  ...
  {
    (void)printf(
        "Contoso published display 20x4 interface failed with error %" PRIi32 "\r\n", result);
  }
  else
  {
    (void)printf("Contoso published display 20x4 interface with success\r\n");
  }
}

void contoso_display_200401_destroy(void)
{
  az_result result;
  ...
  {
    (void)printf(
        "Contoso unpublished display 20x4 interface failed with error %" PRIi32 "\r\n", result);
  }
  else
  {
    (void)printf("Destroy Contoso producer for display 20x4.\r\n");
  }
}
