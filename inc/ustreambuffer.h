 ...
 *
 *          -@b true - To inform the new uStreamBuffer that it shall take the ownership of the memory. This 
 ...
 *                  change its content since this buffer now belongs to the uStreamBuffer.
 *          -@b false - To inform the new uStreamBuffer that it shall not take the ownership of the memory.
 *                  This means that the uStreamBuffer shall allocate its own memory and copy the content of
 ...
 *                              If {@code true} the uStreamBuffer will take ownership of the memory allocated for the
...
/**
 * @brief   Factory to create a new uStreamBuffer using constant content, which cannot be changed or released.
 *
 * <p> This factory creates a uStreamBuffer that manages content in constant memory. In this case, 
 *      this API will not copy the content to a internal buffer, or release the memory when the uStreamBuffer 
 *      is disposed. To create a new uStreamBuffer, a valid pointer to a constant memory address shall be provided, 
 ...
 *      next one starts.
 * <p> Copying a uStreamBufferMulti creates a new instance that has the same content of the original one
 ...
 * <p> Because a uStreamBufferMulti exposes a uStreamBuffer interface, it can be appended to another
 *      uStreamBufferMulti.
 * <p> Release part of a uStreamBufferMulti will dispose the appended buffers that are not necessary anymore,
 *      and the disposal of the uStreamBufferMulti will dispose all appended buffers.
 *
 ...
 *      the immutability, every copy of the uStreamBufferMulti will create an independent instance of it, which
 *      means that appending a new buffer in a current instance of the uStreamBufferMulti will not affect the
 *      other instances.
 *
