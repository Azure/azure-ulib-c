...
#define UCONTRACT_REQUIRE_EQUALS(val, expected, result) \
    do { \
        if(val != expected) \
        { \
            ULIB_CONFIG_LOG(AZIOT_ULOG_TYPE_ERROR, AZIOT_ULOG_REQUIRE_EQUALS_STRING, MU_TOSTRING(val), MU_TOSTRING(expected)); \
            return result; \
        } \
    } while((void)0,0)
...
#define UCONTRACT_REQUIRE_NOT_EQUALS(val, expected, result) \
    do { \
        if(val == expected) \
        { \
            ULIB_CONFIG_LOG(AZIOT_ULOG_TYPE_ERROR, AZIOT_ULOG_REQUIRE_NOT_EQUALS_STRING, MU_TOSTRING(val), MU_TOSTRING(expected)); \
            return result; \
        } \
    } while((void)0,0)
...
#define UCONTRACT_REQUIRE_NOT_NULL(val, result) \
    do { \
        if(val == NULL) \
        { \
            ULIB_CONFIG_LOG(AZIOT_ULOG_TYPE_ERROR, AZIOT_ULOG_REQUIRE_NOT_NULL_STRING, MU_TOSTRING(val)); \
            return result; \
        } \
    } while((void)0,0)
