...
/**
 * @brief   Signature of the function to release inner buffer control block
 * 
 * @param[in]   void*       void pointer to memory that needs to be free'd
 * 
 * @return  void
 */
typedef void (*USTREAM_INNER_BUFFER_RELEASE_CALLBACK)(void*);
 ...
 *
 * @param[in,out]   ustream_return                     The {@link USTREAM}* to initialize.
 ...
 * @param[in]       release_buffer              The #USTREAM_BUFFER_RELEASE_CALLBACK function that will be called for the inner 
 ...
 * @param[in]       release_inner_control_block The #USTREAM_INNER_BUFFER_RELEASE_CALLBACK function that will be called for the inner buffer once 
 *                                              all the references to the uStream are released. If <tt>NULL</tt> is passed, the data is assumed 
 *                                              to be constant with no need to be free'd. In other words, there is no need for notification 
 *                                              that the memory can be released. As a default, users may use the standard <tt>free</tt> to 
 *                                              release malloc'd memory.
 *
 * @return The {@link AZIOT_ULIB_RESULT} with the uStream interface.
 *          @retval     ULIB_SUCCESS                If the uStreamMulti appended the provided uStream with success.
 */
MOCKABLE_FUNCTION(, AZIOT_ULIB_RESULT, ustream_create,
        USTREAM*, ustream_return,
        const uint8_t* const, buffer,
        size_t, buffer_length,
        USTREAM_BUFFER_RELEASE_CALLBACK, release_buffer,
        USTREAM_INNER_BUFFER_RELEASE_CALLBACK, release_inner_control_block);
