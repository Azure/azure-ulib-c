...
static az_ulib_result concrete_read(az_ulib_ustream* ustream_instance, uint8_t* const buffer, size_t buffer_length, size_t* const size);
static az_ulib_result concrete_get_remaining_size(az_ulib_ustream* ustream_instance, size_t* const size);
static az_ulib_result concrete_get_position(az_ulib_ustream* ustream_instance, offset_t* const position);
static az_ulib_result concrete_release(az_ulib_ustream* ustream_instance, offset_t position);
static az_ulib_result concrete_clone(az_ulib_ustream* ustream_instance_clone, az_ulib_ustream* ustream_instance, offset_t offset);
...
{
    az_ulib_ustream_multi_data_cb* multidata = (az_ulib_ustream_multi_data_cb*)ustream_instance->control_block->ptr;
        ...
        offset_t position)
{
    ...
    AZ_ULIB_UCONTRACT(AZ_ULIB_UCONTRACT_REQUIRE(!AZ_ULIB_USTREAM_IS_NOT_TYPE_OF(ustream_instance, api),
                                            AZ_ULIB_ILLEGAL_ARGUMENT_ERROR, AZ_ULIB_ULOG_USTREAM_ILLEGAL_ARGUMENT_ERROR_STRING));
        ...
        size_t* const size)
{
    ...
    az_ulib_ustream_multi_data_cb* multi_data = (az_ulib_ustream_multi_data_cb*)ustream_instance->control_block->ptr;
    az_ulib_ustream* current_ustream = (ustream_instance->inner_current_position < multi_data->ustream_one.length) ?
    ...
    *size = 0;
            ...
            (current_ustream != NULL))
    {
        ...
        intermediate_result = az_ulib_ustream_read(current_ustream, &buffer[*size], remain_size, &copied_size);
...
static az_ulib_result concrete_clone(az_ulib_ustream* ustream_instance_clone, az_ulib_ustream* ustream_instance, offset_t offset)
{
    ...
    AZ_ULIB_UCONTRACT(AZ_ULIB_UCONTRACT_REQUIRE(!AZ_ULIB_USTREAM_IS_NOT_TYPE_OF(ustream_instance, api),
                    ...
                    AZ_ULIB_UCONTRACT_REQUIRE((offset <= (UINT32_MAX - ustream_instance->length)), AZ_ULIB_ILLEGAL_ARGUMENT_ERROR, "offset exceeds max size"));
...
static void ustream_multi_init(az_ulib_ustream* ustream_instance, az_ulib_ustream_data_cb* control_block,
                                    az_ulib_ustream_multi_data_cb* multi_data, az_ulib_release_callback multi_data_release)
{
...
az_ulib_result az_ulib_ustream_concat(
    az_ulib_ustream* ustream_instance,
    ...
    az_ulib_release_callback multi_data_release)
{
    ...
    AZ_ULIB_UCONTRACT(AZ_ULIB_UCONTRACT_REQUIRE_NOT_NULL(ustream_instance, AZ_ULIB_ILLEGAL_ARGUMENT_ERROR),
                    AZ_ULIB_UCONTRACT_REQUIRE_NOT_NULL(ustream_to_concat, AZ_ULIB_ILLEGAL_ARGUMENT_ERROR),
                    AZ_ULIB_UCONTRACT_REQUIRE_NOT_NULL(multi_data, AZ_ULIB_ILLEGAL_ARGUMENT_ERROR));
    ...
    if((result = az_ulib_ustream_clone(&multi_data->ustream_two, ustream_to_concat, ustream_instance->length)) == AZ_ULIB_SUCCESS)
    {
        size_t remaining_size;
        if((result = az_ulib_ustream_get_remaining_size(&(multi_data->ustream_two), &remaining_size)) == AZ_ULIB_SUCCESS)
        {
...
az_ulib_result az_ulib_ustream_split(
    az_ulib_ustream* ustream_instance,
    ...
    offset_t split_pos)
{
    ...
    AZ_ULIB_UCONTRACT(AZ_ULIB_UCONTRACT_REQUIRE_NOT_NULL(ustream_instance, AZ_ULIB_ILLEGAL_ARGUMENT_ERROR),
                 AZ_ULIB_UCONTRACT_REQUIRE_NOT_NULL(ustream_instance_split, AZ_ULIB_ILLEGAL_ARGUMENT_ERROR));

    az_ulib_result result;

    offset_t old_position;
    /*[az_ulib_ustream_split_get_position_failed]*/
    if((result = az_ulib_ustream_get_position(ustream_instance, &old_position)) == AZ_ULIB_SUCCESS)
    {
        ...
        else
        {
            ...
            if((result = az_ulib_ustream_get_remaining_size(ustream_instance, &ustream_remaining_size)) == AZ_ULIB_SUCCESS)
            {
                ...
                else
                {
                    ...
                    if((result = az_ulib_ustream_set_position(ustream_instance, split_pos)) == AZ_ULIB_SUCCESS)
                    {
                        /*[az_ulib_ustream_split_clone_failed]*/
                        if((result = az_ulib_ustream_clone(ustream_instance_split, ustream_instance, split_pos)) == AZ_ULIB_SUCCESS)
                        {
                            /*[az_ulib_ustream_split_set_position_second_failed]*/
                            if((result = az_ulib_ustream_set_position(ustream_instance, old_position)) == AZ_ULIB_SUCCESS)
                            {
